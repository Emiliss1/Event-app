...
function Unverified() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:3000/posts").then((response) => {
      if (response.data.result.length > 0) {
        setposts(pagination(response.data.result, curPage).pageItems);
        setLastPage(pagination(response.data.result).totalPages);
      }
    });
  ...
  const Ids = () => {
    const postsIds = posts.map((post) => {
      const authorsIds = authors.find(
        (authorid) => authorid.username === post.author
      );
      return {
        ...post,
  ...
  const handleAdminCategory = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:3000/admincategory", {
        category,
        postid,
      });
  ...
  const handleAdminRemove = async () => {
    try {
      const response = await axios.post("http://localhost:3000/deletepost", {
        postid,
      });
  ...
  const handleAdminVerify = async () => {
    try {
      const response = await axios.post("http://localhost:3000/adminverify", {
        postid,
      });
  ...
  const handleAdminBan = async () => {
    try {
      const response = await axios.post("http://localhost:3000/adminban", {
        author,
      });
  ...
  const handleAdminUnban = async () => {
    try {
      const response = await axios.post("http://localhost:3000/adminunban", {
        author,
      });
  ...
  return (
    <div className="main">
      ...
      <div className="unverifiedHead">
        <h3 className="eventText">
          <div className="sqr"></div>
          <div className="dec">
            <img src={document} alt="." />
          </div>
          Nepatvirtinti renginiai
        </h3>
      </div>
          ...
          <div className="postWrap" key={post.eventid}>
            <div className="post">
              <div className="userSection">
                <img
                  src={
                    post.authorsIds?.profilepicture ||
              ...
              <div className="postSection">
                <h2>{post.title}</h2>
                <p>
                  <strong>Renginio kategorija:</strong> {post.category}
                </p>
                <p>
                  <strong>Renginio Laikas:</strong> {post.time}
                </p>
                <p>
                  <strong>Renginio Vieta:</strong> {post.place}
                </p>
              ...
              <div>
                ...
                <form onSubmit={handleAdminCategory}>
                  ...
                  <div className="categorywrap">
                    ...
                    <button className="admincategory" type="submit">
                      Redaguoti
                    </button>
                  </div>
                </form>
                <div className="adminBtnWrap">
                  <button className="adminremove" onClick={handleAdminRemove}>
                    Pašalinti tema
                  </button>
                  <button className="adminVerify" onClick={handleAdminVerify}>
                    Patvirtinti tema
                  </button>
                  ...
                  {(() => {
                    if (post.authorsIds && post.authorsIds.banned === 1) {
                      return (
                        <div>
                          <button
                            className="adminUnban"
                            onClick={handleAdminUnban}
                          >
                            Atblokuoti vartotoja
                          </button>
                        </div>
                      );
      ...
      {posts.length > 0 && (
        <div className="pagination">
          <button className="pageBtn" onClick={() => handlePagination("prev")}>
            <GrFormPrevious />
          </button>
          <p className="curPage">
            {curPage + 1}/{lastPage}{" "}
          </p>
          <button className="pageBtn" onClick={() => handlePagination("next")}>
            <GrFormNext />
          </button>
        </div>
      )}
